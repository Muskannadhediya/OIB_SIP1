...
public class ATM<pu1blic>
{
    ...
    public ATM()
    {
        ...
        depositSlot = new Deposit(); // create deposit slot
    ...
    public void authenticateuser(int pin){
        ...
        if (userAuthenticated)
        {
            ...
            if (AdminCheck == 0){
                ...
                createAdminGUI();
            Iterator UserIterator = (Iterator) BankDatabase.createIterator();
    ...
    private <Transaction> void performTransactions(int a)
    {
    ...
    private <Transaction> Transaction createTransaction(int type)
    {
        ...
        if(type == 1) // create new BalanceInquiry transaction
            temp = (Transaction) new BalanceEnquiry(
                    currentAccountNumber, screen, bankDatabase);
        else if(type == 2)// create new Withdrawal transaction
            temp = (Transaction) new Withdrawal(currentAccountNumber, screen,
                    bankDatabase, keypad, cashDispenser);
        else if(type == 3){ // create new Deposit transaction
            screen.setSize( 400, 250 );
            temp = (Transaction) new Deposit(currentAccountNumber, screen,
    ...
    public class Nextcheck implements ActionListener
    {
        public void actionPerformed( ActionEvent e )
        {
            ...
            IterateUser((Iterator) BankDatabase.createIterator());
        }
    }
    //Action listener used for the literator pattern
    public class Prevcheck implements ActionListener
    {
        public void actionPerformed( ActionEvent e )
        {
            ...
            prevIterateUser((Iterator) BankDatabase.createIterator());
        }
    }
   ...
   public void IterateUser(Iterator Iterator){
       ...
       // Display the current user in the GUI message labels.
        Account AccountItem = (Account) Iterator.next();
    ...
    private static class Inputfield2 {
